...
// このファイルが /api/fetch-and-parse.js に置かれている場合、
// https://<プロジェクト名>.vercel.app/api/fetch-and-parse でアクセスできる
// Vercelはこのファイルの「デフォルトエクスポート(module.exports自体)」が
// 関数であることを期待するため、他の関数はそのプロパティとして生やす。
async function handler(req, res) {
  try {
    const data = await main();
    if (!data) {
      res.status(200).json({ message: "本日は対象データなし", races: [], entries: [] });
      return;
    }

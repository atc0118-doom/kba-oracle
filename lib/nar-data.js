...
// "計不"(計測不能)のような非数値文字列が混ざっていても
// NaNのままDBに送らず、必ずnullにするための安全な数値変換
...
function mapRaceListRow(row) {
  return {
    ...
    raceNo: toNumberOrNull(row["レース番号"]),
    postTime: row["発走時刻"],
    raceName: row["レース名"],
    raceClass: row["条件"],
    surface: row["芝ダート区分"],
    turnDirection: row["回り"],
    distance: toNumberOrNull(row["距離"]),
    weather: row["天候"] || null,
    condition: row["馬場"] || null,
    numHorses: toNumberOrNull(row["頭数"]),
  };
}
...
function mapHorseListRow(row) {
  return {
    ...
    raceDate: toIsoDate(row["競走年月日"]),
    raceNo: toNumberOrNull(row["レース番号"]),
    frameNo: toNumberOrNull(row["枠番"]),
    horseNo: toNumberOrNull(row["馬番"]),
    horseName: row["馬名"],
    sex: row["性"],
    age: toNumberOrNull(row["齢"]),
    jockeyName: row["騎手名"],
    jockeyAffiliation: row["騎手所属"],
    weightCarried: toNumberOrNull(row["負担重量"]),
    trainerName: row["調教師"],
    horseWeight: toNumberOrNull(row["馬体重"]),
    weightDiff: toNumberOrNull(row["馬体重増減"]),
    finishPosition: toNumberOrNull(row["着順"]),
    time: row["タイム"] || null,
    popularity: toNumberOrNull(row["人気"]),
    // 予測ロジック(lib/predict.js)で使う実績データ。"1-0-2-3"形式の文字列のまま保持する
    overallRecord: row["全成績"] || null,
    courseRecord: row["当競馬場成績"] || null,
    distanceRecord: row["うち当距離成績"] || null,
    jockeyRecord: row["騎手成績"] || null,
  };
}
...
function mapOddsRow(row) {
  return {
    ...
    oddsValue: toNumberOrNull(row["オッズ"]),
    popularity: toNumberOrNull(row["人気"]),
  };
}
...
function mapPaybackRow(row) {
  ...
  if (row["単勝組番"]) {
    payouts.push({
      ...
      payoutAmount: toNumberOrNull(row["単勝払戻金（円）"]),
      popularity: toNumberOrNull(row["単勝人気"]),
    });
  }
  for (let i = 1; i <= 3; i++) {
    if (row[`複勝組番${i}`]) {
      payouts.push({
        ...
        payoutAmount: toNumberOrNull(row[`複勝払戻金${i}（円）`]),
        popularity: toNumberOrNull(row[`複勝人気${i}`]),
      });
    }
  }
  if (row["３連単組番馬番1"]) {
    payouts.push({
      ...
      payoutAmount: toNumberOrNull(row["３連単払戻金（円）"]),
      popularity: toNumberOrNull(row["３連単人気"]),
    });
  }
  return payouts;
}
